...
def get_db_config(config_file, DB_NAME):
    ...
    if v_db_type == 'oracle':
        ...
        exit(1)
    db_config = {'db_type': v_db_type, 'db_host': v_db_host, 'db_port': v_db_port, 'db_cfg': v_db_cfg,
                 'db_username': v_db_username, 'db_password': v_db_password}
    return db_config
...
def get_connect(db_config):
    ...
    if v_db_type == 'oracle':
        ...
        connection = pymysql.connect(host=v_db_host, port=v_db_port, user=v_db_username, passwd=v_db_password,
                                     db=v_db_cfg)
    return connection
